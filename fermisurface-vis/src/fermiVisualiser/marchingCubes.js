...
export function marchingCubes(dims, potential, bounds) {
  ...
  for (let z = 0; z < dims[2] - 1; z++) {
    const sz = scale[2] * z + shift[2];
    for (let y = 0; y < dims[1] - 1; y++) {
      const sy = scale[1] * y + shift[1];
      for (let x = 0; x < dims[0] - 1; x++) {
        const sx = scale[0] * x + shift[0];
        ...
        for (let i = 0; i < 12; i++) {
          ...
          p0.set(cubeVerts[e[0]]);
          p1.set(cubeVerts[e[1]]);
          ...
          nv[0] = sx + scale[0] * (p0[0] + t * (p1[0] - p0[0]));
          nv[1] = sy + scale[1] * (p0[1] + t * (p1[1] - p0[1]));
          nv[2] = sz + scale[2] * (p0[2] + t * (p1[2] - p0[2]));
